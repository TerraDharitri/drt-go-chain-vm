...
func TestExecution_TransferDCDTNFTExecute_NoFailExecution_WithReturnData(t *testing.T) {
	testConfig := makeTestConfig()
	testConfig.DCDTTokensToTransfer = 100
	...
	_, err := test.BuildMockInstanceCallTest(t).
			...
			test.CreateMockContract(test.ParentAddress).
				...
				WithMethods(func(parentInstance *mock.InstanceMock, config interface{}) {
					parentInstance.AddMockMethod("callChild", func() *mock.InstanceMock {
						...
						returnVal := vmhooks.TransferDCDTNFTExecuteWithTypedArgsWithFailure(
							host,
							[]byte(test.ChildAddress),
							[]*vmcommon.DCDTTransfer{
								&vmcommon.DCDTTransfer{
		...
		WithSetup(func(host vmhost.VMHost, world *worldmock.MockWorld) {
			...
			_ = childAccount.SetTokenBalanceUint64(test.DCDTTestTokenName, 0, testConfig.DCDTTokensToTransfer)
			createMockBuiltinFunctions(t, host, world)
			setZeroCodeCosts(host)
		}).
		WithInput(test.CreateTestContractCallInputBuilder().
...
func TestExecution_TransferDCDTNFTExecute_NoFailExecution_WithError(t *testing.T) {
	testConfig := makeTestConfig()
