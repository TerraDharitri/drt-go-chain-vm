...
func failIfMoreThanOneDCDTTransfer(context *VMHooksImpl) bool {
	...
	if len(runtime.GetVMInput().DCDTTransfers) > 1 {
		FailExecution(context.GetVMHost(), vmhost.ErrTooManyDCDTTransfers)
		return true
	}
	return false
}
...
func (context *VMHooksImpl) GetDCDTNFTNameLength(
	...
	if dcdtData == nil || dcdtData.TokenMetaData == nil {
		FailExecution(context.GetVMHost(), vmhost.ErrNilDCDTData)
		return 0
	}
...
func (context *VMHooksImpl) TransferValue(
	...
	if host.IsBuiltinFunctionCall(data) {
		context.FailExecution(vmhost.ErrTransferValueOnDCDTCall)
		return 1
	}
...
func TransferValueExecuteWithTypedArgs(
	...
	if contractCallInput != nil {
		if host.IsBuiltinFunctionName(contractCallInput.Function) {
			FailExecution(host, vmhost.ErrNilDCDTData)
			return 1
		}
	}
	...
	if host.IsBuiltinFunctionCall([]byte(data)) &&
		...
		FailExecution(host, vmhost.ErrTransferValueOnDCDTCall)
		return 1
	}
...
func TransferDCDTNFTExecuteWithTypedArgs(
	...
	return TransferDCDTNFTExecuteWithTypedArgsWithFailure(host, dest, transfers, gasLimit, function, data, true)
}

// TransferDCDTNFTExecuteWithTypedArgsWithFailure defines the actual transfer DCDT execute logic
func TransferDCDTNFTExecuteWithTypedArgsWithFailure(
	host vmhost.VMHost,
	dest []byte,
	transfers []*vmcommon.DCDTTransfer,
	gasLimit int64,
...
func (context *VMHooksImpl) CheckNoPayment() {
	...
	if vmInput.CallValue.Sign() > 0 {
		context.FailExecution(vmhost.ErrNonPayableFunctionRewa)
		return
	}
	if len(vmInput.DCDTTransfers) > 0 {
		context.FailExecution(vmhost.ErrNonPayableFunctionDcdt)
		return
	}
}
