...
func TestRustBasicFeaturesBarnard(t *testing.T) {
	...
	ScenariosTest(t).
		...
		Exclude("features/basic-features/scenarios/get_shard_of_address.scen.json").
		ReplacePath(
			"../output/basic-features.drtsc.json",
			filepath.Join(getTestRoot(), "features/basic-features/output/basic-features-barnard.drtsc.json"),
		).
		Run().
		CheckNoError()
}
...
func TestRustPayableFeaturesBarnard(t *testing.T) {
	...
	ScenariosTest(t).
		Folder("features/payable-features/scenarios").
		ReplacePath(
			"../output/payable-features.drtsc.json",
			filepath.Join(getTestRoot(), "features/payable-features/output/payable-features-barnard.drtsc.json"),
		).
		Run().
		CheckNoError()
}
...
func TestForwarderTransfExecFallibleMultiReject(t *testing.T) {
	ScenariosTest(t).
		Folder("features/composability/scenarios").
		File("forw_raw_transf_exec_fallible_multi_rewa_reject.scen.json").
		Run().
		CheckNoError()
}
